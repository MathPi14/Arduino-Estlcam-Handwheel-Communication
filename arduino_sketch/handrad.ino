...
void buffer_data() {
  ...
  speed_data = SPEED_GPIO_INVERTED ? 65535 - (analogRead(SPEED_GPIO)/1024 * 65535) : analogRead(SPEED_GPIO)/1024 * 65535;
  feed_data = FEED_GPIO_INVERTED ? 65535 - (analogRead(FEED_GPIO)/1024 * 65535) : analogRead(FEED_GPIO)/1024 * 65535;
  x_data = analogRead(JOYSTICK_X_GPIO)/1024 * 65535; // convert the returning 10 bit uint from analogRead() to a 16 bit uint 
  y_data = analogRead(JOYSTICK_Y_GPIO)/1024 * 65535;
  z_data = analogRead(JOYSTICK_Z_GPIO)/1024 * 65535;
}
